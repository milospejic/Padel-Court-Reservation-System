...
import org.springframework.web.reactive.resource.NoResourceFoundException; // WebFlux specific
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleServerWebInputException(ServerWebInputException ex){
        // Replaces MissingServletRequestParameterException
    ...
    public ResponseEntity<?> handleMethodNotAllowedException(MethodNotAllowedException ex){
        // Replaces HttpRequestMethodNotSupportedException
    ...
    public ResponseEntity<?> handleInvalidRequestException(ServiceUnavailableException ex){
        return ResponseEntity.status(503).body(
                new ExceptionModel(503, HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage())
        );
    }
}